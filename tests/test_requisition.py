...
class TestRequisitionApi:
    ...
    def test_get_requisitions(self, requisition: RequisitionsApi, client: NordigenClient):
        """
        ...
        with patch('requests.get') as mock_request:
            mock_request.return_value.json.return_value = generate_mock(self.requisition_id)
            response = requisition.get_requisitions()

            assert len(response["results"]) == 2
            assert response["results"][0]["id"] == "3fa85f64-5717-4562-b3fc-2c963f66afa6"
            assert mock.call(
    ...
    def test_delete_requisition(
        self, requisition: RequisitionsApi
    ):
        """
